...
class ComfyUIImageProvider:
    ...
    async def _wait_for_output(self, client: httpx.AsyncClient, prompt_id: str) -> dict[str, str]:
        ...
        while time.monotonic() < deadline:
            response = await client.get(f"{self.config.base_url.rstrip('/')}/history/{prompt_id}")
            response.raise_for_status()
            ...
            for node_output in outputs.values() if isinstance(outputs, dict) else []:
                ...
                if images and isinstance(images[0], dict) and isinstance(images[0].get("filename"), str):
                    return images[0]
            await asyncio.sleep(self.config.poll_interval_seconds)
        raise ImageGenerationError("comfyui_generation_timeout")


def _inject_inputs(workflow: dict[str, Any], *, prompt: str, width: int, height: int) -> None:
    ...
    for node in workflow.values():
        ...
        if class_type in {"CLIPTextEncode", "CLIPTextEncodeSDXL", "TextEncodeQwenImageEdit"}:
            inputs["text"] = prompt
