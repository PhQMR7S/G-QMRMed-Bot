...
class Settings(BaseSettings):
    ...
    database_url: str = Field(default="postgresql+asyncpg://gqmrmed:gqmrmed@postgres:5432/gqmrmed", alias="DATABASE_URL")
    ...
    ai_provider_order: str = Field(default="openai,gemini_free,openrouter_free,groq_free,huggingface_free,ollama,openai_compatible", alias="AI_PROVIDER_ORDER")
    ...
    def validate_production_secrets(self) -> "Settings":
        """Reject unsafe production defaults and non-canonical image dimensions."""
        if self.image_width != 1024 or self.image_height != 1536:
            raise ValueError("GQMRMed production image canvas must be exactly 1024x1536")
