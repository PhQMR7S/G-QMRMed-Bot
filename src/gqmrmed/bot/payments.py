    ...
    async with session.begin():
        ...
        pack = (
            await session.execute(
                select(CreditPack).where(CreditPack.code == pack_code, CreditPack.is_active.is_(True))
            )
        ).scalar_one_or_none()
        if pack is None:
...
async def stars_plan_callback(callback: CallbackQuery, session: AsyncSession) -> None:
    ...
    async with session.begin():
        ...
        plan = (
            await session.execute(
                select(Plan).where(Plan.code == plan_code, Plan.is_active.is_(True), Plan.code != PlanCode.FREE.value)
            )
        ).scalar_one_or_none()
