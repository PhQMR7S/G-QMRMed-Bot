...
def _credit_keyboard(packs: Sequence[CreditPack]) -> InlineKeyboardMarkup:
    rows = [[InlineKeyboardButton(text=f"{pack.credits} تصاميم · {pack.stars_price} Stars", callback_data=f"credits:{pack.code}")] for pack in packs]
...
async def credits_handler(message: Message, session: AsyncSession) -> None:
    await emoji_settings(session)
    result = await session.execute(select(CreditPack).where(CreditPack.is_active.is_(True)).order_by(CreditPack.stars_price.asc()))
    packs = result.scalars().all()
    lines = ["<b>حصص التصميم</b>", "", "اشترِ رصيد تصاميم إضافياً بدون اشتراك.", "الرصيد لا يستبدل الخطة؛ ويُستخدم بعد استنفاد الحصة اليومية المتاحة.", ""]
    for pack in packs:
        per_design = pack.stars_price / pack.credits
        lines.append(f"<b>{pack.name}</b> · {pack.credits} تصاميم · {pack.stars_price} Stars · {per_design:.2f} Stars/تصميم")
...
async def payment_support_handler(message: Message) -> None:
    await message.answer("لدعم المدفوعات، أرسل رقم العملية أو تفاصيل المشكلة إلى دعم GQMRMed عبر @ID29i.\nلا ترسل كلمات مرور أو مفاتيح سرية أو بيانات بطاقة كاملة.")
...
async def credit_menu_callback(callback: CallbackQuery, session: AsyncSession) -> None:
    ...
    result = await session.execute(select(CreditPack).where(CreditPack.is_active.is_(True)).order_by(CreditPack.stars_price.asc()))
    packs = result.scalars().all()
    await callback.message.edit_text("<b>حصص التصميم الإضافية</b>\n\nاختر الحصة التي تريد شراءها:", parse_mode="HTML", reply_markup=_credit_keyboard(packs))
...
async def credit_terms_callback(callback: CallbackQuery, session: AsyncSession) -> None:
    ...
    if isinstance(callback.message, Message):
        await callback.message.edit_text(TERMS_TEXT, parse_mode="HTML", reply_markup=_terms_keyboard())
    await callback.answer()
...
async def credit_pack_callback(callback: CallbackQuery, session: AsyncSession) -> None:
    ...
    async with session.begin():
        user = await provision_user(session, telegram_id=callback.from_user.id, username=callback.from_user.username, first_name=callback.from_user.first_name, last_name=callback.from_user.last_name, language=callback.from_user.language_code)
        if not user.is_active:
            await callback.answer("الحساب غير نشط.", show_alert=True)
            return
        user.terms_accepted_at = datetime.now(UTC)
        pack = (await session.execute(select(CreditPack).where(CreditPack.code == pack_code, CreditPack.is_active.is_(True)))).scalar_one_or_none()
        if pack is None:
            await callback.answer("الحصة غير متاحة حالياً.", show_alert=True)
            return
        payload = f"gqmrmed:credit:{pack.code}:{uuid4().hex}"
        await create_stars_credit_payment(session, user_id=user.id, credit_pack=pack, invoice_payload=payload)
    await callback.answer("تم تجهيز الفاتورة.")
    await callback.message.answer_invoice(title=f"GQMRMed {pack.name}", description=f"حصة إضافية: {pack.credits} تصاميم بدون مدة انتهاء تلقائية.", payload=payload, provider_token="", currency="XTR", prices=[LabeledPrice(label=pack.name, amount=pack.stars_price)])
...
async def stars_plan_callback(callback: CallbackQuery, session: AsyncSession) -> None:
    ...
    async with session.begin():
        ...
        if not user.is_active:
            ...
            return
        user.terms_accepted_at = datetime.now(UTC)
        plan = (await session.execute(select(Plan).where(Plan.code == plan_code, Plan.is_active.is_(True), Plan.code != PlanCode.FREE.value))).scalar_one_or_none()
    ...
    await callback.answer("تم تجهيز الفاتورة.")
    await callback.message.answer_invoice(title=f"GQMRMed {plan.name}", description=f"اشتراك {plan.name}: {plan.daily_limit} تصاميم يومياً لمدة {plan.duration_days} يوماً.", payload=payload, provider_token="", currency="XTR", prices=[LabeledPrice(label=plan.name, amount=plan.stars_price)])
...
async def pre_checkout_handler(query: PreCheckoutQuery, session: AsyncSession) -> None:
    ...
    stale = payment is not None and payment.created_at is not None and datetime.now(UTC) - payment.created_at > timedelta(minutes=30)
    if payment is None or stale or payment.provider != "telegram_stars" or payment.status != PaymentStatus.PENDING.value or payment.stars_amount != query.total_amount or query.currency != "XTR":
...
async def successful_payment_handler(message: Message, session: AsyncSession) -> None:
    ...
    try:
        async with session.begin():
            payment = await finalize_stars_payment(session, invoice_payload=successful.invoice_payload, transaction_id=successful.telegram_payment_charge_id, telegram_user_id=message.from_user.id, total_amount=successful.total_amount)
    except ValueError:
        await message.answer("تم استلام إشعار دفع غير متوافق مع الطلب. لم يتم منح الاشتراك أو الرصيد تلقائياً؛ تواصل مع /paysupport.")
        return
    if payment.credit_pack_id is not None:
        await message.answer("تم شراء حصة التصاميم بنجاح.\n" f"تمت إضافة الرصيد إلى حسابك. رقم العملية: {payment.transaction_id}\n" "سيُستخدم الرصيد تلقائياً بعد استنفاد الحصة اليومية المتاحة.")
    else:
        await message.answer("تم استلام الدفع وتفعيل الاشتراك بنجاح.\n" f"رقم العملية: {payment.transaction_id}\n" "يمكنك الآن البدء بإرسال طلبات التصميم.")
