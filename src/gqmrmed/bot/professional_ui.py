...
def _emoji(settings: dict[str, str], slot: str, fallback: str = "") -> str:
    """Render a bound Telegram Premium Emoji; never fall back to Unicode emoji."""
...
def _settings_map(rows: list[SystemSetting]) -> dict[str, str]:
    return {row.key: row.value for row in rows if row.key.startswith(EMOJI_PREFIX)}
...
async def _emoji_settings(session: AsyncSession) -> dict[str, str]:
    ...
    return _settings_map(list(rows))
        ...
        rows.append(
            [
                InlineKeyboardButton(
                    text="لوحة الإدارة",
                    url=f"{get_settings().admin_panel_url}/admin/panel",
                )
            ]
        )
...
async def _pro_user(session: AsyncSession, message: Message) -> User:
...
async def _home_text(session: AsyncSession) -> str:
    ...
    return (
        ...
        f"{_emoji(e, 'medical')} منصة لتحويل المحتوى الطبي إلى تصميم بصري منظم، مع البحث والتحقق وبناء المحتوى قبل الإخراج النهائي.\n\n"
        f"{_emoji(e, 'free')} <b>الخطة المجانية</b> · 3 تصاميم يومياً\n"
        "يمكنك البدء مباشرة، أو اختيار خطة أو شراء رصيد تصاميم إضافي."
    )
...
async def _plans_text(session: AsyncSession) -> str:
    ...
    lines.append("الدفع للخدمات الرقمية داخل Telegram يتم عبر Telegram Stars.")
    return "\n".join(lines)
...
TERMS_TEXT = (
    ...
    "<b>الرصيد المشتَرى</b>\n"
    "رصيد التصاميم الإضافي مستقل عن الاشتراك ولا تنتهي صلاحيته تلقائياً. كل تصميم ناجح يستهلك وحدة واحدة؛ وإذا فشل التوليد قبل إتمامه تُعاد الوحدة المحجوزة.\n\n"
    "<b>الدفع</b>\n"
    "لأن الخدمة رقمية داخل Telegram، تتم المشتريات داخل Telegram باستخدام Telegram Stars (XTR). لا نعتمد على لقطة شاشة أو إثبات يدوي؛ لا يُمنح الاشتراك أو الرصيد إلا بعد تحقق الخادم من successful_payment.\n\n"
    "<b>الدعم والاسترداد</b>\n"
    "للمشكلات المتعلقة بالدفع استخدم /paysupport. أي طلب استرداد أو نزاع تتم مراجعته وفق حالة العملية وسجلات Telegram والدعم.\n\n"
...
PAYMENT_TEXT = (
    ...
    "3. ستظهر فاتورة Telegram الرسمية بالمبلغ المحدد.\n"
    "4. أكمل الدفع من داخل Telegram.\n"
    "5. يتحقق الخادم من successful_payment ثم يفعّل الاشتراك أو يضيف الرصيد تلقائياً.\n\n"
    "<b>مهم</b>\n"
    "لا ترسل كلمة مرور أو رمزاً سرياً أو بيانات بطاقة كاملة إلى الدعم. لقطة الشاشة وحدها ليست إثباتاً لمنح الخدمة.\n\n"
    "للدعم: /paysupport"
)
...
HELP_TEXT = (
    ...
    "إذا فشل الطلب قبل إتمامه، تتم تسوية الحصة المحجوزة وفق نظام الاستخدام."
)
...
async def professional_start(message: Message, session: AsyncSession) -> None:
    async with session.begin():
        user = await _pro_user(session, message)
    ...
    await message.answer(
        ...
        parse_mode="HTML",
        reply_markup=_main_keyboard(_owner(message.from_user.id)),
    )
...
async def pro_home(callback: CallbackQuery, session: AsyncSession) -> None:
    if callback.message is not None:
        await callback.message.edit_text(
            await _home_text(session),
            parse_mode="HTML",
            reply_markup=_main_keyboard(_owner(callback.from_user.id)),
        )
...
async def pro_generate(callback: CallbackQuery, session: AsyncSession) -> None:
    ...
    if callback.message is not None:
        await callback.message.edit_text(
            ...
            "سيتم إدخال الطلب في مسار المعالجة، ثم يصلك التصميم النهائي عند اكتماله.",
            parse_mode="HTML",
...
async def pro_support(callback: CallbackQuery, session: AsyncSession) -> None:
    ...
    if callback.message is not None:
        await callback.message.edit_text(
            ...
            "للمشكلات العامة أرسل وصف المشكلة مع رقم العملية إن كانت متعلقة بالدفع.\n\n"
...
async def pro_credits(callback: CallbackQuery, session: AsyncSession) -> None:
    ...
    rows = [
        [InlineKeyboardButton(text=f"شراء {pack.credits} تصاميم · {pack.stars_price} Stars", callback_data=f"credits:{pack.code}")]
        for pack in packs
    ]
    rows.extend(
        [
...
async def pro_account(callback: CallbackQuery, session: AsyncSession) -> None:
    ...
    expiry_text = "مستمر" if expires is None else expires.astimezone(UTC).strftime("%Y-%m-%d %H:%M UTC")
    e = await _emoji_settings(session)
    text = (
        f"{_emoji(e, 'medical')} <b>حسابي</b>\n\n"
        f"الخطة الحالية: <b>{entitlement.plan.name}</b>\n"
        f"الاستخدام اليومي: {used}\n"
        f"محجوز حالياً: {reserved}\n"
...
async def professional_plans_command(message: Message, session: AsyncSession) -> None:
    await message.answer(
        ...
        reply_markup=InlineKeyboardMarkup(
            inline_keyboard=[
                [InlineKeyboardButton(text="شراء PLUS", callback_data="stars:PLUS"), InlineKeyboardButton(text="شراء PRO", callback_data="stars:PRO")],
                [InlineKeyboardButton(text="شراء حصص التصميم", callback_data="credits:menu")],
            ]
        ),
    )
...
def _extract_command_slot(message: Message) -> str | None:
    parts = (message.text or "").split(maxsplit=1)
    if len(parts) != 2:
        return None
    slot = parts[1].strip().split(maxsplit=1)[0]
...
async def emoji_bind(message: Message, session: AsyncSession) -> None:
    if message.from_user is None or not _owner(message.from_user.id):
        await message.answer("هذا الأمر متاح لمالك البوت فقط.")
        return
    slot = _extract_command_slot(message)
    emoji_id = _extract_custom_emoji_id(message)
    if slot is None or emoji_id is None:
        slots = ", ".join(EMOJI_SLOTS)
        await message.answer(
            "صيغة الربط:\n/emoji_bind <slot> <Premium Emoji>\n\n"
            f"الـslots المتاحة: {slots}\n\n"
            "أرسل الأمر مع الـPremium Emoji نفسه حتى يلتقط Telegram الـcustom_emoji_id الحقيقي."
        )
        return
...
async def owner_admin(message: Message) -> None:
    ...
    await message.answer(
        ...
        reply_markup=InlineKeyboardMarkup(
            inline_keyboard=[
                [InlineKeyboardButton(text="فتح لوحة الإدارة", url=f"{get_settings().admin_panel_url}/admin/panel")],
                [InlineKeyboardButton(text="القائمة الرئيسية", callback_data="pro:home")],
            ]
        ),
    )
