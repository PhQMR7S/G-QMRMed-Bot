...
async def _emoji_settings(session: AsyncSession) -> dict[str, str]:
    rows = (await session.execute(select(SystemSetting).where(SystemSetting.key.like(f"{EMOJI_PREFIX}%")))).scalars().all()
...
def _main_keyboard(owner: bool) -> InlineKeyboardMarkup:
    rows = [
        [InlineKeyboardButton(text="إنشاء تصميم", callback_data="pro:generate"), InlineKeyboardButton(text="الخطط والاشتراكات", callback_data="pro:plans")],
        [InlineKeyboardButton(text="حصص التصميم", callback_data="pro:credits"), InlineKeyboardButton(text="حسابي ورصيدي", callback_data="pro:account")],
        [InlineKeyboardButton(text="طريقة الاستخدام", callback_data="pro:help"), InlineKeyboardButton(text="الدفع والفوترة", callback_data="pro:payment")],
        [InlineKeyboardButton(text="الشروط والأحكام", callback_data="pro:terms"), InlineKeyboardButton(text="الدعم", callback_data="pro:support")],
    ]
    if owner:
...
def _back_keyboard() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="القائمة الرئيسية", callback_data="pro:home")]])


async def _user_from_message(session: AsyncSession, message: Message) -> User:
    if message.from_user is None:
        raise ValueError("telegram_user_required")
    return await provision_user(session, telegram_id=message.from_user.id, username=message.from_user.username, first_name=message.from_user.first_name, last_name=message.from_user.last_name, language=message.from_user.language_code)


async def _home_text(session: AsyncSession) -> str:
    e = await _emoji_settings(session)
    return f"{_emoji(e, 'brand')} <b>GQMRMed</b>\n<i>Medical Design Intelligence</i>\n\n{_emoji(e, 'medical')} منصة متخصصة لتحويل المحتوى الطبي إلى تصميم بصري منظم، مع البحث والتحقق وبناء المحتوى قبل الإخراج النهائي.\n\n{_emoji(e, 'free')} <b>FREE</b> · 3 تصاميم يومياً\nابدأ مباشرة، أو اختر اشتراكاً، أو اشترِ رصيد تصاميم إضافياً."


async def _plans_text(session: AsyncSession) -> str:
    e = await _emoji_settings(session)
    plans = (await session.execute(select(Plan).where(Plan.is_active.is_(True)).order_by(Plan.price.asc(), Plan.code.asc()))).scalars().all()
    ...
    for plan in plans:
        ...
        lines.extend([f"{_emoji(e, slot)} <b>{plan.name}</b>", f"الحد اليومي: {limit} · المدة: {duration}", f"السعر: {stars}", ""])
...
async def professional_start(message: Message, session: AsyncSession) -> None:
    ...
    if not user.is_active:
        ...
        return
    await message.answer(await _home_text(session), parse_mode="HTML", reply_markup=_main_keyboard(_is_owner(message.from_user.id)))


@router.callback_query(F.data == "pro:home")
async def pro_home(callback: CallbackQuery, session: AsyncSession) -> None:
    if callback.message is not None:
        await callback.message.edit_text(await _home_text(session), parse_mode="HTML", reply_markup=_main_keyboard(_is_owner(callback.from_user.id)))
...
async def pro_generate(callback: CallbackQuery, session: AsyncSession) -> None:
    e = await _emoji_settings(session)
    if callback.message is not None:
        await callback.message.edit_text(f"{_emoji(e, 'create')} <b>إنشاء تصميم طبي</b>\n\nأرسل الآن الموضوع أو النص أو الصورة أو الملف أو الصوت أو الفيديو.\n\nسيُدخل الطلب في مسار المعالجة ثم يصلك التصميم النهائي عند اكتماله.", parse_mode="HTML", reply_markup=_back_keyboard())
    await callback.answer()
...
async def pro_help(callback: CallbackQuery, session: AsyncSession) -> None:
    e = await _emoji_settings(session)
    if callback.message is not None:
        await callback.message.edit_text(f"{_emoji(e, 'research')} {HELP_TEXT}", parse_mode="HTML", reply_markup=_back_keyboard())
    await callback.answer()
...
async def pro_terms(callback: CallbackQuery, session: AsyncSession) -> None:
    e = await _emoji_settings(session)
    if callback.message is not None:
        await callback.message.edit_text(f"{_emoji(e, 'warning')} {TERMS_TEXT}", parse_mode="HTML", reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="الخطط والشراء", callback_data="pro:plans")], [InlineKeyboardButton(text="الدفع والفوترة", callback_data="pro:payment")], [InlineKeyboardButton(text="القائمة الرئيسية", callback_data="pro:home")]]))
    await callback.answer()
...
async def pro_payment(callback: CallbackQuery, session: AsyncSession) -> None:
    e = await _emoji_settings(session)
    if callback.message is not None:
        await callback.message.edit_text(f"{_emoji(e, 'plans')} {PAYMENT_TEXT}", parse_mode="HTML", reply_markup=_back_keyboard())
    await callback.answer()
...
async def pro_support(callback: CallbackQuery, session: AsyncSession) -> None:
    e = await _emoji_settings(session)
    if callback.message is not None:
        await callback.message.edit_text(f"{_emoji(e, 'support')} <b>الدعم</b>\n\nللدعم المتعلق بالدفع استخدم /paysupport.\nللمشكلات العامة أرسل وصف المشكلة ورقم العملية إن كانت متعلقة بالدفع.\n\nلا ترسل كلمات مرور أو مفاتيح سرية أو بيانات بطاقة كاملة.", parse_mode="HTML", reply_markup=_back_keyboard())
    await callback.answer()
...
async def pro_plans(callback: CallbackQuery, session: AsyncSession) -> None:
    if callback.message is not None:
        await callback.message.edit_text(await _plans_text(session), parse_mode="HTML", reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="شراء PLUS", callback_data="stars:PLUS"), InlineKeyboardButton(text="شراء PRO", callback_data="stars:PRO")], [InlineKeyboardButton(text="شراء حصص التصميم", callback_data="credits:menu")], [InlineKeyboardButton(text="الشروط والأحكام", callback_data="pro:terms")], [InlineKeyboardButton(text="القائمة الرئيسية", callback_data="pro:home")]]))
    await callback.answer()
...
async def pro_credits(callback: CallbackQuery, session: AsyncSession) -> None:
    ...
    e = await _emoji_settings(session)
    packs = (await session.execute(select(CreditPack).where(CreditPack.is_active.is_(True)).order_by(CreditPack.stars_price.asc()))).scalars().all()
    lines = [f"{_emoji(e, 'design')} <b>حصص التصميم</b>", "", "شراء رصيد إضافي بدون اشتراك:", ""]
    rows = []
    for pack in packs:
    ...
    rows.extend([[InlineKeyboardButton(text="الشروط والأحكام", callback_data="pro:terms")], [InlineKeyboardButton(text="القائمة الرئيسية", callback_data="pro:home")]])
    if callback.message is not None:
...
async def pro_account(callback: CallbackQuery, session: AsyncSession) -> None:
    ...
    usage = (await session.execute(select(DailyUsage).where(DailyUsage.user_id == user.id, DailyUsage.usage_date == date.today()))).scalar_one_or_none()
    ...
    e = await _emoji_settings(session)
    text = f"{_emoji(e, 'medical')} <b>حسابي ورصيدي</b>\n\nالخطة الحالية: <b>{entitlement.plan.name}</b>\nالاستخدام اليومي: {used}\nالمحجوز حالياً: {reserved}\nالمتاح اليوم: {remaining}\nرصيد التصاميم الإضافي: <b>{user.design_credits}</b>\nانتهاء الاشتراك: {expiry_text}"
    if callback.message is not None:
...
async def professional_support_command(message: Message) -> None:
    await message.answer("<b>دعم المدفوعات</b>\n\nأرسل رقم العملية ووصف المشكلة إلى @ID29i.\nلا ترسل كلمات مرور أو مفاتيح سرية أو بيانات بطاقة كاملة.", parse_mode="HTML")
...
async def emoji_status(message: Message, session: AsyncSession) -> None:
    ...
    for slot in EMOJI_SLOTS:
        lines.append(f"{slot}: {'مرتبط' if settings.get(f'{EMOJI_PREFIX}{slot}') else 'غير مرتبط'}")
